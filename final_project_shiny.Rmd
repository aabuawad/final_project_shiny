...
# selectInput widget
# set school based on lead level
renderUI({
  high_school = all_data %>% filter(lead_med == input$lead_choice) %>% 
                           distinct(school_name) %>% pull()
  
  low_school = all_data %>% filter(lead_med == input$lead_choice) %>% 
                           distinct(school_name) %>% pull()
  
school_choice = all_data %>% 
  filter(!is.na(longitude),
         !is.na(latitude)) %>%
  distinct(school_name_x) %>% 
  pull()

selectizeInput("school_choice", label = h3("Select School"),
            choices = school_choice, selected = school_choice, multiple = TRUE,
...
```{r}
 renderLeaflet({all_data %>%
  filter(lead_med == input$lead_choice,
         school_name_x == input$school_choice) %>% 
